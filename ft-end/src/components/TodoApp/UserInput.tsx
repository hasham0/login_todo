...
export default function UserInput({}: Props) {
  ...
  const getAllTodos = async () => {
    const responce = await fetch("http://localhost:7000/allTodos", {
      method: "GET",
      mode: "cors",
      headers: {
  ...
  const getData = async (data: todoTs) => {
    ...
    const responce = await fetch("http://localhost:7000/newTodo", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  const handleClearAll = async () => {
    const responce = await fetch("http://localhost:7000/clear", {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  const handleDelete = async (id: string) => {
    const responce = await fetch(`http://localhost:7000/deleteTodo/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
